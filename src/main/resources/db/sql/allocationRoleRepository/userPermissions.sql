...
(WITH ordered_lineage (allocation_id, rownum) AS (
    ...
    business_status_filter (role_name, permissions) AS (
    SELECT role_name,
        COALESCE ((
            SELECT CASE
                WHEN business.status = 'CLOSED'
                    THEN array(SELECT unnest(permissions) intersect SELECT unnest(array['READ', 'VIEW_OWN']::allocationpermission[]))
                WHEN business.status = 'SUSPENDED'
                    THEN array[]::allocationpermission[]
                ELSE permissions
            END
            FROM business
            WHERE id IN (
            ...
            OR (:userId = '00000000-0000-0000-0000-000000000000'
                AND id = :businessId)
        ), array[]::allocationpermission[]) as permissions
    FROM business_roles
    )
 ...
 (
    SELECT *
    FROM (
        SELECT DISTINCT ON (roles.user_id) -- only the closest record to the leaf for the user
            ...
            FROM (
                SELECT -- this clause fetches explicit AllocationPermissions
                    ...
                    business_status_filter.permissions,
                    ordered_lineage.rownum
                FROM user_allocation_role INNER JOIN ordered_lineage
                        ON ordered_lineage.allocation_id = user_allocation_role.allocation_id
                    INNER JOIN users
                        ON users.id = user_allocation_role.user_id
                    INNER JOIN allocation
                        ON user_allocation_role.allocation_id = allocation.id
                    INNER JOIN business_status_filter
                        ON user_allocation_role.role = business_status_filter.role_name
                ...
                SELECT -- this clause fetches implicit allocationPermissions for root allocation owners
                    ...
                    (
                        SELECT permissions
                        FROM business_status_filter
                        WHERE role_name = 'Admin')         AS permissions,
                    ordered_lineage.rownum
                FROM allocation INNER JOIN ordered_lineage
                        ON ordered_lineage.allocation_id = allocation.id
                    INNER JOIN users
                        ON (users.business_id = allocation.business_id
                            AND (
            ...
            ORDER BY roles.user_id ASC, roles.rownum DESC) AS "roles0"  -- sort by allocation, leaf to root
    WHERE :userId = '00000000-0000-0000-0000-000000000000'
        OR (
            roles0.user_id = :userId
            AND (
                roles0.user_business_id = roles0.allocation_business_id
                OR (global_permissions && ARRAY [ :crossBusinessBoundaryPermission ]::GlobalUserPermission[])
            )
        )
    )
) -- with
