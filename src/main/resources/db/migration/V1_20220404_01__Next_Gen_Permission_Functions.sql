...
DROP FUNCTION IF EXISTS is_grantee_business_owner_on_grantor(the_grantee grantee, the_grantor grantor);
...
-- Get all the allocation records within the business that the specified user has access to, along with the specific permissions they have at each allocation.
-- If no allocation permissions, but global permissions exist, a single record will be returned with null values for nearly everything except for global permissions.
-- If no allocation or global permissions exist, nothing will be returned.
CREATE OR REPLACE FUNCTION get_all_allocation_permissions(in_business_id UUID, in_user_id UUID, global_roles VARCHAR[])
    RETURNS TABLE (
...
$$
    ...
    BEGIN
        ...
        INTO the_grantor
        FROM business
        WHERE business.id = in_business_id
        AND business.status != business_status_suspended;
        ...
        RETURN QUERY WITH RECURSIVE permissions_hierarchy(
            ...
            SELECT in_user_id, in_business_id, the_grantee.user_type, the_grantee.first_name, the_grantee.last_name,
            ...
            LEFT JOIN user_allocation_role root_assignment ON root.id = root_assignment.allocation_id AND root_assignment.user_id = in_user_id
            LEFT JOIN allocation_role_permissions_for_business(in_business_id) root_definition ON root_assignment.role = root_definition.role_name
            WHERE root.business_id = in_business_id
            ...
            LEFT JOIN user_allocation_role assignment ON walker.id = assignment.allocation_id AND assignment.user_id = in_user_id
            LEFT JOIN allocation_role_permissions_for_business(in_business_id) definition ON assignment.role = definition.role_name
        )
        SELECT hierarchy.*
    ...
    END;
$$;
ALTER FUNCTION get_all_allocation_permissions(in_business_id UUID, in_user_id UUID, global_roles VARCHAR[]) OWNER TO postgres;

-- Combines the above function with all users within a business to return all the permissions for all the allocations that all the users within that
-- business have access to. Please note that due to the fact that it is retrieving permissions for a lot of users at once, it ignores global roles/permissions.
CREATE OR REPLACE FUNCTION get_all_allocation_permissions_for_all_users_in_business(in_business_id UUID)
    RETURNS TABLE (
        user_id UUID, business_id UUID, user_type VARCHAR, first_name BYTEA, last_name BYTEA,
        allocation_id UUID, parent_allocation_id UUID, ordinal INT, role_name VARCHAR, inherited BOOLEAN, permissions allocationpermission[], global_permissions globaluserpermission[]
    )
...
$$
    BEGIN
        ...
        FROM users
        JOIN get_all_allocation_permissions(in_business_id, users.id, ARRAY[]::VARCHAR[]) permissions ON permissions.user_id = users.id
        WHERE users.business_id = in_business_id
        ORDER BY permissions.user_id, permissions.ordinal, permissions.allocation_id ASC;
    END;
$$;
ALTER FUNCTION get_all_allocation_permissions_for_all_users_in_business(in_business_id UUID) OWNER TO postgres;

-- Dropping the old version of this function
DROP FUNCTION IF EXISTS getAllocationPermissions(UUID, UUID, VARCHAR(20));
...
$$
    ...
    FROM get_all_allocation_permissions(in_business_id, in_user_id, global_roles) hierarchy
