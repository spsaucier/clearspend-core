...
-- Dropping old version of this function
DROP FUNCTION IF EXISTS get_all_allocation_permissions(in_business_id UUID, in_user_id UUID, global_roles VARCHAR[]);
...
$$
    DECLARE
        ...
        has_cross_business_permission BOOLEAN;
    BEGIN
        IF in_business_id IS NULL AND in_allocation_id IS NULL THEN
            RAISE EXCEPTION 'Cannot retrieve permissions without either a Business ID or an Allocation ID';
        END IF;

        IF in_user_id IS NULL THEN
            RAISE EXCEPTION 'Cannot retrieve permissions for User without a User ID';
        END IF;
...
-- Dropping old function that is being replaced
DROP FUNCTION get_all_allocation_permissions_for_all_users_in_business(in_business_id UUID);
