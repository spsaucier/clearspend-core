...
public class NumberCrypto {
  ...
  public NumberCrypto(Environment env, JdbcTemplate jdbcTemplate) {
    ...
    panKey =
        BaseEncoding.base16().decode(env.getProperty(fpeEnvPrefix + "pan.key", ""));
    panTweak =
        BaseEncoding.base16().decode(env.getProperty(fpeEnvPrefix + "pan.tweak", ""));
  }
  ...
  public Integer generatePan() {
    Integer nextval =
        jdbcTemplate.queryForObject(
            "select nextval('pan_seq')", Integer.class);
  ...
  private Integer fpeDecrypt(byte[] key, byte[] tweak, Integer cipherText) {
    ...
    return Integer.parseInt(
        formatPreservingEncryption.decrypt(cipherText.toString(), tweak));
  }
}
